...
void SchemeNTRU::encrypt(Ctxt_NTRU& ct, const int b) const
{
    ...
    // g[0] += b * parNTRU.delta_base; // delta_base = q/4
    g[0] += b * round(1.0*parNTRU.delta_base/parNTRU.n_party); // Khanh, 
...
int SchemeNTRU::decrypt(const Ctxt_NTRU& ct) const
{
    ...
    // Khanh
    // output = parNTRU.mod_q_base(output); // r = c.sk(mod q)
    // output = int(round(double(output)/double(parNTRU.q_base)*Param::t)); // (?) int(4r/q) ~ round(2r/q) mod 2
    // cout << "Decryption time: " << float(clock()-start)/CLOCKS_PER_SEC << endl;
    return output;
}
...
void SchemeNTRU::bootstrap(Ctxt_NTRU& ct) const
{
    ...
    ModQPoly acc(N, round(1.0*half_delta_boot/parNTRU.n_party)); // Khanh; half_delta_boot=Q/8
    ...
    for (auto it = acc.begin(); it < acc.end(); ++it)
        // *it += half_delta_boot;
        *it += round(1.0*half_delta_boot/parNTRU.n_party); // Khanh; 
