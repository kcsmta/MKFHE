...
void SchemeLWE::encrypt(Ctxt_LWE& ct, int m) const
{
    ...
    // int b = parLWE.delta_base*m + static_cast<int>(round(gaussian_sampler(rand_engine))); // delta_base*m + e, where delta_base = q/4
    int b = round(1.0*parLWE.delta_base*m/parLWE.n_party) + static_cast<int>(round(gaussian_sampler(rand_engine))); // Khanh
    for (int i = 0; i < n; i++)
    {
        b -= sk_base[i] * a[i]; // -<a, s> + delta_base*m + e
    } 
    parLWE.mod_q_base(b);
    ct.b = b;
...
void SchemeLWE::bootstrap(Ctxt_LWE& ct) const
{
    ...
    // vector<int> acc(N, half_delta_boot); // half_delta_boot = Q/8
    vector<int> acc(N, round(1.0*half_delta_boot/parLWE.n_party)); // Khanh
    ...
    for (auto it = acc.begin(); it < acc.end(); ++it)
        // *it += half_delta_boot;
        *it += round(1.0*half_delta_boot/parLWE.n_party); // Khanh
